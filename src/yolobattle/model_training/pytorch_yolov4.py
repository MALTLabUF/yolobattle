...
from yolobattle.model_training.profiles import DatasetSpec, TrainProfile
...
def prepare(profile: TrainProfile, output_dir: Path) -> tuple[TrainProfile, Path, Path, Path]:
    ...
    val_frac = float(profile.val_fracs[0]) if profile.val_fracs else 0.20
    ...
    return replace(profile, dataset=dataset, data_path=str(data_path)), model_cfg, train_labels, valid_labels
...
def build_command(profile: TrainProfile, model_cfg: Path, train_labels: Path, valid_labels: Path, output_dir: Path) -> list[str]:
    ...
    return [
        ...
        "--epochs", str(profile.epochs or 100), "--batch", str(profile.batch_size),
        "--subdivisions", str(profile.subdivisions), "--workers", str(min(4, os.cpu_count() or 1)),
        "-optimizer", "sgd", "--seed", str(profile.dataset.split_seed), "--mosaic", "0",
